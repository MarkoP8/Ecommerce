...
def checkout(request):
    if request.method == 'GET':
        ...
        if request.user.is_authenticated:
            user_id = user.User.objects.get(email=request.user.email)
    ...
    if request.method == 'POST':
        if request.user.is_authenticated:
            current_user = user.User.objects.get(email=request.POST['email'])
            ...
            try:
                ...
                cart_order = order.Order.objects.filter(user=current_user, ordered=False).first()
                if cart_order:
